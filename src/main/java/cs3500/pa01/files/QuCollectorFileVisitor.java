...
public class QuCollectorFileVisitor implements FileVisitor<Path> {
  /**
   * A list of SummarizableFiles that has been collected so far by the FileVisitor
   */
  private final ArrayList<QuestionFile> files = new ArrayList<>();
  ...
  public FileVisitResult visitFile(Path file, BasicFileAttributes attr) throws IOException {
    ...
    if (fileName.endsWith(".md")) {
      files.add(new MarkdownParser(FileIo.readFile(file.toAbsolutePath().toString())).parse(fileName, attr.creationTime(),
          attr.lastModifiedTime()));
    }

    if (fileName.endsWith(".sr")) {
      files.add(new SrParser(FileIo.readFile(file.toAbsolutePath().toString())).parse(fileName, attr.creationTime(),
          attr.lastModifiedTime()));
    }
   ...
   */
  public ArrayList<QuestionFile> getFiles() {
    return files;
  }
}
